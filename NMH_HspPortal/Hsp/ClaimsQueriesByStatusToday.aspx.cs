...
{
    public partial class ClaimsQueriesByStatusToday : System.Web.UI.Page
    {
        ...
        protected void btnSaveComment_Click(object sender, EventArgs e)
        {
            ...
            /** using (SqlConnection connection = new SqlConnection(connectionString))
             {
                 using (SqlCommand command = new SqlCommand("spUpdateAdviceHspQuery", connection))
                 {
                     command.CommandType = CommandType.StoredProcedure;
                     try
                     {
                         connection.Open();
                         command.Parameters.Add("@BatchNo", SqlDbType.Int).Value = ViewState["adviceBatchNo"].ToString();
                         command.Parameters.Add("@HspQuery", SqlDbType.VarChar).Value = txtComment.Text;
                         command.Parameters.Add("@RowID", SqlDbType.Int).Value = Convert.ToInt32(hfAdviceID.Value);
                         ...
                         {
                             ScriptManager.RegisterStartupScript(this, this.GetType(), "", "toastr.error('Sorry, you cannot query this batch. Current status is NOT Awarded', 'Error');", true);
                         }
                     }
                     catch (Exception ex)
                     {
        ...
        protected bool SendMail(string batchno)
        {
            ...
            try
            {
                ...
                SqlCommand sqlCommand = new SqlCommand("select ClaimsNo, DateOfAttendance, Diagnosis, TariffName, Claimed, ApprovedAmount, Tariff_X_RejectionComments, HspQuery from Vw_AdviceSlip_Outcome where claimsbatchno = '" + batchno + "' and hspquery is not null", sqlConnection); //select query command  
        ...
        {
            Response.Redirect("/Hsp/ClaimsReceivedByStatusToday.aspx?batchStatusId=" + ViewState["batchStatusId"].ToString());
        }
    }
}